...
function Home() {
    ...
    const check = async () => {
        ...
        try {
            const response = await fetch('https://netflixcloneserver-production.up.railway.app/protected-route', {
                method: 'GET',
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`https://netflixcloneserver-production.up.railway.app/userList/${email}`);
        ...
        try {
            const response = await axios.get(`https://netflixcloneserver-production.up.railway.app/userHistory/${email}`);
            const data = response.data;
        ...
        const handleSearch = async () => {
            try {
                if (searchTerm.trim().length === 0) {
                    ...
                    console.log(searchTerm.trim().length);
                    const response = await axios.post('https://netflixcloneserver-production.up.railway.app/content/search', { searchTerm });
